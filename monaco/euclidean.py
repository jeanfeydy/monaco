...
class GaussianProposal(Proposal):
    ...
    def nlog_density(self, target, source, log_weights, scales, probas=None):
        ...
        if probas is None:
            ...
            return (
                -(logdensities_i.view(-1, len(probas)) + probas.log()[None, :])
                .logsumexp(dim=1)
                .view(-1)
            )
...
class GMMProposal(Proposal):
    ...
    def __init__(self, *args, n_classes=5, covariance_type="full", n_iter=10, **kwargs):
        ...
        self.means = 0.5 + 0.5 * torch.randn(self.K, self.D).type(self.dtype)
        ...
        if covariance_type == "full":
            ...
            for k in range(self.K):
                self.covariances[k, :, :] = (
                    torch.diag(1 + 0 * torch.rand(self.D).type(self.dtype)) ** 2
                )
    ...
    def adapt(self, x, reg=1e-7):
        ...
        if self.covariance_type == "full":
            for k in range(self.K):
                self.covariances[k, :, :] += (
                    torch.diag(0.01 + 0 * torch.rand(self.D).type(self.dtype)) ** 2
                )
        else:
            for k in range(self.K):
                self.covariances[k, :, :] += (
                    0.01 + 0 * torch.rand(self.D, 1).type(self.dtype)
                ) ** 2
