...
class HyperbolicSpace(object):
    ...
    def __init__(self, dimension=2, dtype=None, resolution=200):
        ...
        ticks = np.linspace(-1, 1, self.resolution + 1)[:-1] + 0.5 / self.resolution
    ...
    def discrepancy(self, a, b):
        ...
        return 0.0  # This feature is not used in the paper
...
class BallProposal(Proposal):
    ...
    def sample_radii(self, N, scales):
        ...
        if self.D == 2:
            ...
            threshold = (radii.exp() - (-radii).exp()) / (
                scales.exp() - (-scales).exp()
            )
